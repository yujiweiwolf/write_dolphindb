...
//        }
    ...
    void DolphindbWriter::WriteQTick(std::string& raw) {
        if (write_tick_step_ == 0) {
            write_tick_step_++;
            string script;
            script += "existsTable(\"" + dbpath_ + "\", `" + tickname_ + ");";
            LOG_INFO << script;
            TableSP result = conn.run(script);
            LOG_INFO << dbpath_ << ", " << tickname_ << ", exist result: " << result->getString() << endl;
        ...
        if (write_tick_step_ == 1) {
            write_tick_step_++;
            tick_btw_ = make_shared<BatchTableWriter>(host_, port_, userId_, password_, true);
            tick_btw_->addTable(dbpath_, tickname_);
            LOG_INFO << "addTable, " << dbpath_ << ", " << tickname_;
        }
        if (write_tick_step_ == 2) {
            auto q = flatbuffers::GetRoot<co::fbs::QTick>(raw.data());
            string code = q->code() ? q->code()->str() : "";
            string name = q->name() ? q->name()->str() : "";
            string underlying_code = q->underlying_code() ? q->underlying_code()->str() : "";
            ...
            tick_btw_->insert(dbpath_, tickname_
